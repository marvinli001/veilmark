...
显性水印、智能伪隐性水印、算法盲水印三合一的轻量 Web 工具。三种水印可以多选叠加，支持批量处理和多模板；导出时可以用自己的 Ed25519 签名身份签发版权证书，任何人都能离线核验。所有处理都在浏览器本地完成，部署为纯静态站点。
...
- **模板**：4 个只读起步模板（社媒分享、作品集防盗、打印防伪、证件/合同），自建模板可导入导出 JSON（默认去除私钥）；素材栏里每张图可以指定自己的模板。
...
| 命令 | 说明 |
| --- | --- |
| `npm test` | 单元测试（载荷、CDP 抗攻击、伪隐性、排版、模板、实时预览、证书、PNG iTXt、命令行核验） |

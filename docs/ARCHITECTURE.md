...
| 伪隐性水印 | **四层复合**：相位反转光栅（亮度 + 等亮度色度两路）+ 防复印底纹 + 暗部色调 + 等亮度色度，叠加 JND 纹理自适应 | 单一原理只能覆盖一种盗用场景。实测高频层适屏下两区亮度差 ΔY≈1.6 级，最近邻缩放后升到约 12 级（7 倍），光栅色度路再带来约 35 级色差 |
...
| 伪隐性水印 | 预设：日常隐藏 / 打印显形 / 抗截图 / 打印+截图 / 自定义；4 个触发层都可单独开关和调振幅（光栅另有色度振幅）；可设字形加粗、羽化和纹理自适应 | ✅ |
...
ΔY = A·h·[ g·carrier(x,y)·(−s)                      相位反转光栅（亮度路）
         + p·((1−m)·checker + m·coarseDot)          防复印底纹
         + t·s·w_shadow(Y) ]                        暗部色调
ΔRGB = ΔY·(1,1,1) + c·A·s·v_iso                     v_iso 满足 0.299r+0.587g+0.114b = 0
     + min(g_c, room/2)·carrier(x,y)·(−s)·v_by      相位反转光栅（色度路），v_by 为蓝–黄等亮度轴
room = min(B 与 0/255 的距离, R、G 与 0/255 的距离 / v_by.r)，两相位对称，保证不截断
最后加入 TPDF 抖动再取整：亚灰阶振幅在 8-bit 量化后仍能保住均值
```

**光栅色度路。** 与亮度路同一载波、同样相位反转，只是方向换成蓝–黄等亮度轴。适屏缩略时照样被面积平均掉；1:1 下，奈奎斯特附近的蓝–黄色度远超出 S 视锥的空间分辨率，比同幅度的亮度棋盘更难察觉；一旦被最近邻重采样，两区分别落到“偏蓝”和“偏黄”，变成成片的色差。所以它不乘 JND 图（纹理掩蔽图按亮度估计，与高频色度的可见性无关，平坦区恰恰是显形后最容易读的地方），只受通道余量限制。余量必须两相位对称：与防复印底纹叠加时两区各通道的振幅不同，一旦单侧截断，均值偏移就会在适屏下露出字形（单测覆盖了近白、近黑和饱和蓝/黄）。

**字形加粗。** 显形后能不能读出来，除了幅度还取决于字形：几级到十几级的对比度下，细笔画（尤其中文）会糊成一片。`bold` 用白色描边给掩膜加粗（每侧宽度 = 字号 × bold），打印显形与打印+截图默认 3%。

| 触发层 | 针对的盗用场景 | 注意 |
| --- | --- | --- |
| 相位反转光栅 | 重采样、打印、锐化 | 需要无损导出；色度路只对重采样有效（打印加网与网点扩大只看振幅不看相位，两区一样） |
...
**打印+截图（hybrid）。** 四层共用一张掩膜（字号、加粗、羽化只有一份）和同一份“日常看不见”的余量，所以场景是单选，不能把两个预设直接叠加。hybrid 是折中配比：光栅（含色度路 16 级）与防复印底纹和“打印显形”完全一致，暗部色调和色度拉到接近“抗截图”（2.1 / 7 级），字号 11%、羽化 2.5‰ 取两者之间，加粗 3%。加粗让低频层的实心笔画变多，适屏略更显眼，所以 tone/chroma 从 2.2/7.5 降到 2.1/7，保持适屏可见度不高于抗截图。合成图（`npm run bench:glance` 无参数，900px 适屏，ΔY/ΔC）：

| 模式 | PSNR | 适屏可见度 | 对比度 +120% | 饱和度 ×3 | 最近邻 50% | 打印模拟 |
| --- | --- | --- | --- | --- | --- | --- |
| 打印显形 | 28.9 | 1.6/1.9 | 3.1/4.9 | 1.6/5.4 | 11.5/**35.8** | 7.4/1.8 |
| 抗截图 | 37.8 | 3.1/3.7 | 6.0/13.7 | 2.7/14.2 | 3.1/3.8 | 3.7/3.7 |
| 打印+截图 | 29.1 | 3.0/3.6 | 5.9/11.9 | 2.9/13.1 | 12.3/**35.3** | 9.6/3.6 |

加入光栅色度路之前，最近邻 50% 的色差只有 1.7（打印显形）/ 3.7（打印+截图），显形只靠约 12 级的亮度差，在纹理多、饱和度高的商品图上只能看到隐约字影。PSNR 下降约 2 dB 基本来自色度路的能量，适屏可见度不变。
...
4. **1:1 放大时能察觉细纹。** 提供了振幅滑杆和放大镜，由用户在隐蔽和显形之间自行取舍。光栅色度路在 1:1 下基本看不出，但在放大镜里是彩色棋盘格。
5. **打印模拟的显形最弱。** 模拟模型按局部高频能量估计网点扩大，画面本身纹理重（拉丝金属、密集文字）时水印的那份高频被淹没；平坦区又被 JND 图压低了振幅。想更明显只能调高防复印底纹振幅（实测 4 → 6 打印显形约 +20%，1:1 细纹同步变重）。
...
SliderRow pointerdown ──► store.interacting = "glance" | "visible"
  ...
  │   加粗、显性图层 → live Worker.prepareLive()              │
...
tests/                     vitest：payload / blind / glance / layout / templates / live / certificate / png-meta（66 例）
```
...
```bash
npm test                 # vitest：66 例（载荷、CDP 7 种攻击、伪隐性不可见/显形、排版、模板、实时预览分类、证书、签名身份、PNG iTXt、命令行核验）
